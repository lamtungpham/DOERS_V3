        ...
        {entries.map((post) => (
          ...
          >
            ...
            <div className="bg-neutral-100 mb-4 h-48 w-full border border-neutral-200 p-2 handdrawn-border-2">
              {post.image ? (
                <img src={post.image} alt={post.title} loading="lazy" width="400" height="200" className="w-full h-full object-cover transition-all duration-500 rounded-sm" />
              ) : (
            ...
            <Link to="#" aria-label={`Xem chi tiết nhật ký: ${post.title}`} className="inline-flex items-center text-sm font-bold border-b border-brand-dark pb-0.5 hover:text-brand-orange hover:border-brand-orange transition-colors uppercase tracking-widest mt-auto">
